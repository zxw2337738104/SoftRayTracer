...
public:
    ...
    inline Vector3f eval(const Vector3f &wi, const Vector3f &wo, const Vector3f &N, const bool &isDirectLight = true);
...
Vector3f Material::eval(const Vector3f &wi, const Vector3f &wo, const Vector3f &N, const bool &isDirectLight){
    switch(m_type){
        case DIFFUSE:
        {
        ...
        case MICROFACET:
        {
            ...
            if (cosalpha > 0.f && cosbeta > 0.f) {
                ...
                auto G_function = [&]( const Vector3f& v, const Vector3f& N)
                    {
                        float NdotV = dotProduct(N, v);
                        return NdotV / ((1.f - k) * NdotV + k);
                    };
                ...
                if (divisor < EPSILON)specular = Vector3f(1.f);
                else specular = F * G * D / divisor;
