...
BVHBuildNode* BVHAccel::recursiveBuild(std::vector<Object*> objects)
{
    ...
    if (objects.size() == 1) {
        ...
        if (splitMethod == SplitMethod::SAH) {
            ...
            int proper_cut = 0;
            double mintime = 0x3f3f3f;//0x3f3f3f可作无穷大使用
            for (int index = 1; index < bullets_num; ++index) {
                auto middling = objects.begin() + size * index / bullets_num;
            ...
            auto middling = objects.begin() + size * proper_cut / bullets_num;
